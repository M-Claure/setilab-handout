...
// Parallel version of convolution and power estimate. 
// The signal is split into num_threads parts and each 
// thread runs conv_worker on a different part.
// The master thread sums up the partial results to get the final power estimate.
int convolve_and_compute_power_parallel(int length, double input_signal[], int order, double coeffs[], double* power, int num_threads, int num_processors);
